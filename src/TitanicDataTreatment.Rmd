...
cat("The OR between two Parch  and zero Parch ", odds.two.Parch  / odds.zero.Parch , "\n")
cat("The OR between one Parch  and two Parch ", odds.one.Parch  / odds.two.Parch , "\n")
cat("The OR between one Parch  and zero Parch ", odds.one.Parch  / odds.zero.Parch , "\n")
```
...
cat("The OR between embarking in Q and embarking in S ", odds.Q  / odds.S , "\n")
cat("The OR between embarking in C and embarking in Q ", odds.C  / odds.Q , "\n")
cat("The OR between embarking in C and embarking in S ", odds.C  / odds.S , "\n")
```
...
#### 4.3.2 Analisis de la relación entre haber sobrevivido, o no, con las variables numéricas

Para estudiar si existe relación entre las variables numéricas **Age** y **Fare** con la variable **Survived**, se crea un modelo de regresión logística para cada una de las variables y poder evaluar si se trata de variables relevantes para predecir **Survived**.
...
Si observamos el valor p de la variable **Age**, en la tabla de coeficientes, podemos ver que se trata de una variable significante al predecir **Survived**, para valores de significancia menores a 0.01.
...
En el caso de **Fare**, en la tabla de coeficientes, observamos un valor p inferior a todos los valores comunes del nivel de significancia. Esto nos indica que la variable es significante a la hora de predecir **Survived**.



### 4.3.3 ¿La media de edad de las mujeres es igual o inferior a la de los hombres?

Nos preguntamos si la media de edad de las mujeres es igual o inferior a la de los hombres.


#### Análisis visual


#TODO: Agrupar ambas densidades men/wmn en un mismo plot
```{r}
ggplot(data=titanic.women, mapping= aes(x=Age)) + geom_density() + scale_x_continuous("Women's age distribution") + ggtitle("Women's age")
```
...
```{r}
ggplot(data=titanic.men, mapping= aes(x=Age)) + geom_density() + scale_x_continuous("Men's age distribution") + ggtitle("Men's age")
```
...
```{r}
model.age.Pclass <- glm(Survived ~ Age + Pclass, family = binomial(link=logit),
                        data=titanic)
...
```
# TODO: Probar de quitar SibSp, las categorias de peso tienen muy pocas observaciones. 
...
#TODO: gráfico de 2 lineas una para cada columna para ver como varia el accuracy en cada prueba. 
```{r}
...
#TODO: 
- Como afectan las variables a la probabilidad de sobrevivir o no. 
- Comentar resultados relevantes del estudio para crear un modelo de regresión. 
