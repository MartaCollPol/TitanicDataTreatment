...
En primer lugar se seleccionan dos subsets diferenciando entre si alguien sobrevive o no.

```{r}
passengers.survive <- subset(titanic, Survived == 1)
passengers.not.survive <- subset(titanic, Survived == 0)
```


A continuación, se separa el dataset por las clases
```{r}
passengers.high.class <- subset(titanic, Pclass == 1)
passengers.med.class <- subset(titanic, Pclass == 2)
passengers.low.class <- subset(titanic, Pclass == 3)
```
...
Nos preguntamos si la media de edad de hombres y mujeres es la misma.
...
Hipótesis alternativa: $H_{1}: \mu_W\neq\mu_M$
