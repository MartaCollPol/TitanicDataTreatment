...
#TODO: Introducir bien el Odds ratio y 
...
Warning causado por categorias con muy pocas observaciones.
...
Para estudiar si existe relación entre las variables numéricas **Age** y **Fare** con la variable **Survived**, se crea un modelo de regresión logística para cada una de las variables.
...
#TODO: Reescrivir -> Existe relción ya que el p-valor es menor que el nivel de significancia.
...
```{r}
var.test(titanic.women$Age, men$Age)
```
...
```{r}
t.test(titanic.women$Age, men$Age, alternative = "l", var.equal = TRUE)
```
