...
Observamos que muchas de estas variables se presentan como Integers o Strings, cuando realmente nos expresan una categoría. Por ello factorizaremos las variables categoricas.
...
En este estudio nos centraremos en responder a la pregunta "¿Qué grupos de personas tuvieron más probabilidades de sobrevivir el accidente?". O dicho de otra forma, "¿Cuáles fueron los factores de peso que influyeron en que los pasajeros tuvieran mayor probabilidad de sobrevivir?".
...
De las variables de interés para el análisis, seleccionaremos una agrupación por genero, la variable **Sex**, para realizar un análisis posteriormente por contraste de hipótesis.
...
Para confirmar si hay relación entre las variables categóricas **Sex**, **Pclass**, **SibSp**, **Parch** y **Embarked** con la variable **Survive** que nos indica si los pasajeros sobrevivieron o no al accidente, utilizaremos el test Chi-cuadrado de Pearson [4].
...
Hay relación, pero observamos un aviso que nos indica que existe cierto error en el resultado. Este aviso es provocado por categorias con muy pocas observaciones[5], lo evaluaremos posteriormente con un análisis visual.
...
Si observamos la variable **Sex**, podemos observar una mayor probabilidad de sobrevivir siendo mujer que siendo hombre. 

Para el caso de de la variable que nos indica la clase socio económica, observamos una gran diferencia entre las probabilidades de sobrevivir en función de la clase. Más de la mitad de los pasajeros de primera clase sobrevivieron, de los pasajeros de segunda clase sobrevivieron aproximadamente la mitad, y para los de clase baja solo un cuarto lo hicieron, siendo esta la clase más numerosa. 

Si nos fijamos ahora en las variables relacionadas con tener familia a bordo, **SibSp** y **Parch**, primero debemos destacar que la mayoría de pasajeros no tenían familiares a bordo, de ellos un poco más de un cuarto sobrevivieron. La segunda categoría más numerosa es tener un hermano o esposa/o a bordo, para el caso de **SibSp**, o tener un hijo o padre a bordo, para **Parch**, en ambos casos observamos que aproximadamente la mitad sobrevivieron, por lo que tener un familiar a bordo parece que influye en la probabilidad de sobrevivir. El mismo caso sucede para tener dos padres o hijos a bordo, aproximadamente la mitad sobreviven. Los demás casos donde hay más de dos familiares a bordo tienen tan pocas observaciones que no se pueden considerar relevantes para extraer conclusiones.

Finalmente comentaremos la variable **Embarked**, que nos indica en que puerto subieron los pasajeros. Observamos que la mayoría de pasajeros subieron en el puerto indicado como S, de los cuales menos de la mitad sobrevivieron. En el caso de pasajeros que subieron en los puertos indicados como C y Q, aproximadamente la mitad sobrevivieron.

Podemos intuir una relación directa con todas las variables categóricas observadas, y la probabilidad de sobrevivir.


##### 4.3.1.3 Odds-Ratio

A continuación, para representar de forma numérica las probabilidades de sobrevivir en función de la categoría de cada variable categórica, utilizaremos el calculo del Odds-Ratio para la variable dependiente **Survived**. El OR es un número que permite conocer cómo varía la probabilidad de que la variable dependiente adquiera un valor en función del valor que adquiera la variable independiente.
...
Para esta variable no tendremos en cuenta las categorías con muy pocas observaciones.
```{r}
# Tabla de frecuencias
...
Observamos que una persona que hubiera embarcado en el puerto Q, tenia 1.24 veces más probabilidades de sobrevivir que una persona que hubiera embarcado en el puerto S.
...
Nos preguntamos si la media de edad de las mujeres es igual o inferior a la de los hombres. Para responder a esta pregunta, realizaremos una prueva estadística por contraste de hipótesis.


#### 4.3.2.1 Análisis visual

Observemos las distribuciones y densidades de las edades en función del genero.
...
En ambas distribuciones observamos un pico pronunciado en los 30 años. La edad de las mujeres va de cero a sesenta y pocos años mientras que la de los hombres va de cero a ochenta. Las distribuciones son parecidas, pero no parecen muy normales pues no tienen forma simétrica de campana de Gauss. 


#### 4.3.2.2 Normalidad y homoscedasticidad


Realizaremos una prueba de normalidad Shapiro-Wilk para determinar si nos encontramos ante distribuciones normales.
...
Recordemos que para los tests de normalidad, la hipótesis nula indica normalidad, mientras que la alternativa indica que las distribuciones no son normales. En ambos casos, obtenemos valores p inferiores a los límites comunes de significancia, por ello, podemos confirmar que no nos encontramos ante distribuciones normales. 

También es relevante mencionar que al disponer de una muestra poblacional suficientemente grande, según el Teorema del Límite Central, la distribución de las medias muéstrales seguirán una distribución normal y por ello podremos aplicar la posterior prueba por contraste de hipótesis.

A continuación, evaluaremos la homoscedasticidad entre las edades de los hombres y las mujeres, para determinar si tienen varianzas iguales o diferentes. La hipótesis nula de este test, nos indica que las varianzas son iguales.
...
Para realizar un estudio por contraste de hipótesis, primeramente plantearemos la pregunta responder, la hipótesis nula y la hipótesis alternativa.
...
Recordemos que podemos considerar las distribuciones normales al aplicar el Teorema del límite Central, y que las varianzas de ambas muestras son iguales. Aplicaremos entonces un test de hipótesis paramétrico de dos muestras independientes sobre la media de edad. Se tratará de un test unilateral por la izquierda, es decir, para rechazar la hipótesis nula, deberemos observar una media de edad suficientemente inferior para las mujeres versus la media de edad de los hombres.
...
Si consideramos un nivel de significancia de $\alpha=0.05$, podemos observar un valor p inferior a $\alpha$, por lo que podremos descartar la hipótesis nula que indica que ambas medias son iguales, y nos decantaremos por la hipótesis alternativa que indica que la media de edad de las mujeres es inferior a la de los hombres. Si nos fijamos con los resultados del test, nos indica que la media de la muestra de las mujeres es de 28.19 mientras que la de los hombres es de 30.49 años de edad.


### 4.4 Modelo de regresión logística

En este apartado, crearemos un modelo de regresión logística, y analizaremos como añadir variables independientes al modelo, nos afecta la precisión de los resultados (*accuracy*).

En primer lugar, se divide el dataset en un dataset de train y otro de test que se usará para evaluar los resultados de los modelos. Realizamos esta partición manteniendo una proporción 3 a 1.
...
En primer lugar, se crea un modelo que utiliza una solo variable para predecir la variable dependiente **Survived**. Evaluaremos la precisión de los modelos. Este proceso nos ayudara a determinar las variables más relevantes para la predicción.
...
Observamos que la variable con más peso, con la que obtenemos un accuracy mayor, es **Sex**, como ya estudiamos previamente, hay una gran diferencia en el porcentaje de mujeres que sobrevivieron, versus el de los hombres.

Partiremos de la variable **Sex**, siendo la que ha obtenido mejores resultados de precisión, e iremos añadiendo variables al modelo con el objetivo de mejorar las predicciones.
...
Se decide añadir ahora la variable **Pclass**, siendo la variable que mejores resultados nos otorga.
...
Observamos que al añadir al modelo la variable **Age**, conseguimos un accuracy de 0.81, siendo esta la variable que mejores resultados aporta, la añadiremos al modelo.
...
Observamos que llegado este punto, la precisión del modelo al añadir las variables restantes **Fare** y **Embarked** disminuye. Por esta razón determinamos que nuestro modelo final es el que incluye las variables, **Sex, SibSp, Pclass y Age**.
...
Con la realización de esta práctica, hemos querido responder a la pregunta, "¿Cuales fueron los factores de peso que influyeron en que los pasajeros del Titanic tuvieran mayor probabilidad de sobrevivir?". 

Partimos del conjunto de datos del Titanic proporcionado por los recursos de la práctica, seleccionamos las variables que describían características de los pasajeros y realizamos un proceso de limpieza y tratamiento de valores vacíos y outliers.

Seguidamente realizamos un estudio de normalidad y homogeniedad de la varianza para las variables numéricas del conjunto de datos **Age** y **Fare**, del cual pudimos determinar que ninguna de ellas sigue una distribución normal, ni las varianzas de **Age** y **Fare**, entre las dos categorías de la variable dependiente estudiada **Survived** son iguales. Es importante mencionar, que el conjunto de datos cuenta con más de 800 observaciones, 

Ni age ni fare son normales ni homojeneas pero aplicamos TLC.

Prueba Chi-cuadrado, todas las variables tienen relación con Survived.

En vista del analisis visual y el Odds-ratio, aprentamente las variables con mayor peso son Sex y Pclass. También observamos variabilidad entre las categorías de las demás variables en cuanto a la probabilidad de sobrevivir, peró aparentemente no son tan significativas.

Nos preguntamos si ¿La media de edad de las mujeres es igual o inferior a la de los hombres? Para responder a esta pregunta realizamos un contraste de hipótesis, y determinamos que con un nivel de significancia del 0.05%, podemos confirarmar que la media de edad de las mujeres es inferior a la de los hombres.

En cuanto a la elaboración de un modelo de regresión logística, se destacan los siguientes resultados. En primer lugar, la variable más determinante a la hora de discernir entre si un pasajero sobrevivirá o no ha sido la variable **Sex**, un resultado esperado en vista de los análisis previos. Sin embargo, la segunda variable a incluir en el modelo ha sido la variable **SibSp**, lo cual era difícilmente predecible. Se han ido añadiendo nuevas variables hasta alcanzar un modelo con una precisión del 81%. En este modelo final estaban incluidas las variables **Sex**, **SibSp**, **Pclass** y **Age**, por lo que se considera que estas cuatro variables son las más determinantes a la hora de clasificar los registros.

Se destaca que gran parte de las variables se han añadido sin los términos de interacción ya que el escueto tamaño del dataset no permitía la creación con tantos términos de interacción.
